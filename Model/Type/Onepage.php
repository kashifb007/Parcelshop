 ...
 */
class Lindybop_Parcelshop_Model_Type_Onepage extends Mage_Checkout_Model_Type_Onepage
{
    /**
    ...
    public function saveBilling($data, $customerAddressId)
    {
        ...
        Mage::getSingleton('core/session')->setIsSubscribed(empty($data['is_subscribed']) ? 0 : 1);
        ...
        if (!$address->getEmail() && $this->getQuote()->getCustomerEmail()) {
            ...
            Mage::getSingleton('core/session')->setCustomerEmail($this->getQuote()->getCustomerEmail());
        }
        else if(!empty($address->getEmail()))
        {
            Mage::getSingleton('core/session')->setCustomerEmail($address->getEmail());            
        }

        $quote = $this->getQuote();

        if(empty(Mage::getSingleton('core/session')->getCustomerEmail()))
        {
            $customer = $quote->getCustomer();
            Mage::getSingleton('core/session')->setCustomerEmail($customer->getEmail());                
        }

        if(empty(Mage::getSingleton('core/session')->getCustomerEmail()))
        {
            Mage::getSingleton('core/session')->setCustomerEmail($quote->getBillingAddress()->getEmail());                
        }
    ...
    public function saveShipping($data, $customerAddressId)
    {
        ...
        Mage::getSingleton('core/session')->setHermesSelected(empty($data['hermes_check']) ? 0 : 1);
